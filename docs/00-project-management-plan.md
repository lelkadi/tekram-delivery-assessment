...
| QA | antigravity / gemini-3.5-flash | Part 2 only: run the API for real, verify every endpoint + edge cases (invalid coupon, out-of-stock, bad JWT), check tests pass. Gemini is a third model family (decorrelated from deepseek engineers and claude reviewers) for maximum gate independence — accepted risk: antigravity is unverified/fails silently, so run the loud-failure spike first ([technical-decisions.md](technical-decisions.md) TD-006). |
