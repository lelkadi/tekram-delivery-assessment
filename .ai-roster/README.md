...
| `qa` | **antigravity / gemini-3.5-flash** | Code issues only: checks out the PR branch read-only in its own worktree, runs the API + tests for real, verdict `6-qa-failed` / `7-qa-passed`. Gemini is a **third model family** — decorrelated from both the deepseek engineers and the claude reviewers, for maximum gate independence. ⚠️ Antigravity runtime is unverified (fails silently) — run the loud-failure spike before trusting verdicts at the gate; see [technical-decisions.md](../docs/technical-decisions.md) TD-006. |
