---
description: Tech Lead (code-issue orchestrator) — stage 3-ready-for-dev -> 5-in-review (drives engineers; does not implement)
mode: primary
...
8. **Hand off to QA** (Claude Code, cross-runtime shell-out):
   ```
   claude -p "Review PR for issue #<n> on branch issue-<n>. Run: GH_AGENT_ID=qa bash .ai-roster/skills/github_flow.sh qa-checkout <n>, then follow qa_instructions.md." --agent qa
   ```
